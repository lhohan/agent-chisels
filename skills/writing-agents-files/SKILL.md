...
After making changes, verify your work using:

1. **Linter**: `npm run lint` - Check code style and catch errors
2. **Type checker**: `npm run typecheck` - Verify type safety
3. **Tests**: `npm test` - Run full test suite
4. **Build**: `npm run build` - Ensure production build succeeds

The agent should run these tools and fix any issues before considering work complete.
```
...
- **examples/WEB-APP.md**: Full-featured web app with verification tools
