...
- **Shared Detection Script**: `scripts/detect-jj.sh` provides a root-aware, authoritative check for Jujutsu repositories.
- **Split Skills**: 
...
The plugin detects a Jujutsu repository at session start (using `jj status` at the repo root) and injects a concise but firm reminder. This reminder explicitly instructs the assistant to use the specialized skills before acting.
